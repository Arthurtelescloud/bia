...
create_task_definition() {
    ...
    if [ $? -ne 0 ] || [ -z "$current_task_def" ]; then
        log_error "Não foi possível obter a task definition atual: $task_family" >&2
        log_error "Verifique se a task definition existe e se você tem permissões adequadas" >&2
        exit 1
    fi
    ...
    if [ ! -s "$temp_file" ]; then
        log_error "Falha ao criar arquivo temporário com task definition" >&2
        rm -f "$temp_file"
        exit 1
    fi
    ...
    if [ $? -ne 0 ] || [ -z "$new_task_def" ]; then
        log_error "Falha ao processar task definition com jq" >&2
        rm -f "$temp_file"
        exit 1
    fi
    ...
    if [ ! -s "$new_temp_file" ]; then
        log_error "Falha ao criar arquivo temporário com nova task definition" >&2
        rm -f "$temp_file" "$new_temp_file"
        exit 1
    fi
    
    # Registrar nova task definition e capturar tanto stdout quanto stderr
    local register_output=$(aws ecs register-task-definition --region $region --cli-input-json file://"$new_temp_file" --output json 2>&1)
    ...
    if [ $register_exit_code -ne 0 ]; then
        ...
        log_error "Erro: $register_output" >&2
        exit 1
    fi
    
    # Extrair revision do output
    local new_revision=$(echo "$register_output" | jq -r '.taskDefinition.revision' 2>/dev/null)
    
    if [ -z "$new_revision" ] || [ "$new_revision" = "null" ]; then
        log_error "Falha ao obter revision da nova task definition" >&2
        log_error "Output do registro: $register_output" >&2
        exit 1
    fi
